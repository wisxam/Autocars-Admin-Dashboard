...
const Sellers = () => {
  ...
  const handleDelete = (id,email) => {
    ...
    const url = "http://127.0.0.1:8000/api/delete_seller"+"/"+id+"/"+email+"/"+subject
...
const handleban = (id,email) => {
  ...
  const url = "http://127.0.0.1:8000/api/baned_seller"+"/"+id+"/"+email+"/"+subject
        fetch( url, { method: 'get'}).then((Response) => {
          if(!Response.ok){
            throw new Error('Something went wrong')
          }
...
// console.log(id);
  ...
  return (
    <Box m="20px">
      <Header title="SELLERS" subtitle="Managing the SELLERS Members" />
      <Box
        m="40px 0 0 0"
        height="75vh"
        sx={{
